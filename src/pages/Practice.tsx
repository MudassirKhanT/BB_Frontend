...
import {
  ChevronDown, ExternalLink, CheckCircle2,
  Circle, ArrowLeft, Code2, BookOpen, Target,
        ...
        <div className="border-t border-slate-100">
          ...
          {group.problems.map((problem, idx) => (
            <div key={problem._id} className="border-t border-slate-50">
              {/* Desktop row */}
              <div
                className={`hidden sm:grid grid-cols-[40px_1fr_110px_80px_80px_90px] gap-2 items-center px-5 py-3 hover:bg-blue-50/40 transition-colors group ${
              ...
              >
                ...
                <div className="flex justify-center">
                  {problem.solutionArticle ? (
                    <a
                      href={problem.solutionArticle}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center gap-1 px-2 py-1 bg-violet-50 border border-violet-200 rounded-lg text-violet-600 hover:bg-violet-100 transition-colors text-[11px] font-semibold"
                      title="Read article"
                    >
