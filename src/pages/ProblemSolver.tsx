...
interface Problem {
  ...
  difficulty: "Easy" | "Medium" | "Hard"; frequency: number;
  description: string; examples: Example[]; testCases: TestCase[];
  starterCode: { python: string; javascript: string; cpp: string; java: string; sql?: string };
  topicTag: string; leetcodeUrl: string; companies?: string[];
  solutionArticle?: string; hiddenTestCaseCount?: number; totalTestCaseCount?: number;
...
function renderDescription(text: string) {
  ...
  return parts.map((part, i) => {
    ...
    if (part.startsWith("`") && part.endsWith("`"))
      return <code key={i} className="px-1.5 py-0.5 bg-slate-100 text-blue-700 rounded text-[0.85em] font-mono">{part.slice(1, -1)}</code>;
    return part.split("\n").map((line, j) => (
...
export default function ProblemSolver() {
  ...
  useEffect(() => {
    ...
    if (!slug) return;
    setLoading(true);
    problemApi.getBySlug(slug).then((data) => {
      setProblem(data);
      ...
      setLoadError(err instanceof Error ? err.message : "Problem not found");
  ...
  const updateSuggestions = (value: string, cursor: number) => {
    ...
    const matches = CODE_SNIPPETS[language].filter(
      (s) => s.trigger.startsWith(word.toLowerCase()) && s.trigger !== word.toLowerCase()
    );
    ...
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <button onClick={() => navigate(-1)} className="mt-4 px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-sm font-semibold">Go Back</button>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-slate-800 border-b border-slate-700 px-4 py-2.5 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button onClick={handleSubmit} disabled={running || submitting} className="bg-emerald-600 hover:bg-emerald-500 text-white text-xs h-8 px-4 gap-1.5">
            {submitting ? <><Loader2 className="w-3.5 h-3.5 animate-spin" />Submitting...</> : <><Check className="w-3.5 h-3.5" />Submit</>}
          </Button>
        </div>
      </div>
        ...
        <div className="w-full h-[45%] md:h-full md:w-[45%] md:min-w-[320px] bg-slate-50 flex flex-col border-b md:border-b-0 md:border-r border-slate-700 overflow-hidden">
          <div className="flex items-center gap-0 border-b border-slate-200 bg-white shrink-0">
            {[
              ...
              <button key={id} onClick={() => setActiveTab(id as "description" | "solution" | "testcases")}
                ...
                <div className="mb-5">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {problem.companies?.slice(0, 3).map((c) => (
                      <span key={c} className="px-1.5 py-0.5 bg-slate-100 text-slate-500 text-[10px] font-medium rounded-md">{c}</span>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="space-y-4">
                    {problem.examples.map((ex, i) => (
                      <div key={i} className="border border-slate-200 rounded-xl overflow-hidden">
                        <div className="px-4 py-2 bg-slate-100 border-b border-slate-200"><span className="text-xs font-bold text-slate-500">Example {i + 1}</span></div>
                        <div className="p-4 space-y-2 bg-white">
                          <div><span className="text-xs font-bold text-slate-400 uppercase">Input</span><pre className="mt-1 text-sm font-mono bg-slate-50 rounded-lg p-3 text-slate-800 whitespace-pre-wrap border border-slate-100">{ex.input}</pre></div>
                          <div><span className="text-xs font-bold text-slate-400 uppercase">Output</span><pre className="mt-1 text-sm font-mono bg-slate-50 rounded-lg p-3 text-slate-800 whitespace-pre-wrap border border-slate-100">{ex.output}</pre></div>
                          {ex.explanation && <div><span className="text-xs font-bold text-slate-400 uppercase">Explanation</span><p className="mt-1 text-sm text-slate-600">{ex.explanation}</p></div>}
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            {activeTab === "solution" && (
              <div className="p-5">
                ...
                {problem.solutionArticle
                  ? <div className="prose prose-slate max-w-none text-sm leading-7" dangerouslySetInnerHTML={{ __html: problem.solutionArticle }} />
                  : <p className="text-sm text-slate-400">No solution article available yet.</p>}
              </div>
            )}
            {activeTab === "testcases" && (
              <div className="p-5">
                      ...
                      {problem.testCases.filter((t) => !t.isHidden).map((tc, i) => (
                        <div key={i} className="border border-slate-200 rounded-xl overflow-hidden bg-white">
                          <div className="flex items-center justify-between px-4 py-2 bg-slate-50 border-b border-slate-100">
                            ...
                            <button onClick={() => { setCustomInput(tc.input); setUseCustomInput(true); setOutputTab("input"); }} className="text-xs text-blue-600 hover:text-blue-700 font-semibold">Use as input</button>
                          </div>
                          <div className="p-4 grid grid-cols-2 gap-3">
                            <div><p className="text-xs font-bold text-slate-400 uppercase mb-1">Input</p><pre className="text-xs font-mono bg-slate-50 rounded-lg p-2.5 text-slate-700 whitespace-pre-wrap border border-slate-100 min-h-[2.5rem]">{tc.input}</pre></div>
                            <div><p className="text-xs font-bold text-slate-400 uppercase mb-1">Expected</p><pre className="text-xs font-mono bg-green-50 rounded-lg p-2.5 text-green-800 whitespace-pre-wrap border border-green-100 min-h-[2.5rem]">{tc.expectedOutput}</pre></div>
                          </div>
                        </div>
                      ))}
                    </div>
        ...
        <div className="w-full h-[55%] md:h-full md:flex-1 flex flex-col overflow-hidden bg-slate-900">
          ...
          <div className="flex items-center justify-between px-4 py-2 bg-slate-800 border-b border-slate-700 shrink-0">
            <div className="flex items-center gap-1 bg-slate-900 rounded-lg p-0.5">
              {LANGUAGES.map((lang) => (
                <button key={lang.id} onClick={() => { setLanguage(lang.id); setSuggestions([]); }}
            ...
            <div className="flex items-center justify-between px-4 border-b border-slate-800 h-9 shrink-0">
              <div className="flex items-center gap-0">
                {[
                  ...
                  <button key={id} onClick={() => { setOutputTab(id as "output" | "input" | "results"); setOutputHeight((h) => h < 200 ? 260 : h); }}
                ...
                {outputTab === "output" && (
                  <div className="p-4">
                    {running ? (
                      ...
                      <p className="text-slate-500 text-sm">Click <span className="text-blue-400 font-semibold">Run</span> to execute and see output here.</p>
                    ) : (
                      <div className="space-y-2">
                        {/* Status badge */}
                        <div className="flex items-center gap-2 mb-3">
                          <span className={`px-2 py-0.5 rounded text-xs font-bold ${
                            output.exitCode === 0 ? "bg-green-900/50 text-green-400 border border-green-700" :
                            "bg-red-900/50 text-red-400 border border-red-700"
                          }`}>
                            {output.status || (output.exitCode === 0 ? "✓ Success" : "✗ Error")}
                          </span>
                ...
                {outputTab === "results" && (
                  <div className="p-4">
                    {submitting ? (
                      ...
                      <p className="text-slate-500 text-sm">Click <span className="text-emerald-400 font-semibold">Submit</span> to run all test cases.</p>
                    ) : (
                      <div>
                        {/* Verdict banner */}
                        <div className={`flex items-center justify-between p-3 rounded-lg mb-3 ${
                          submissionAccepted ? "bg-green-900/40 border border-green-700" :
                          submissionResult.status === "compile_error" ? "bg-yellow-900/40 border border-yellow-700" :
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className={`font-black text-sm ${submissionAccepted ? "text-green-400" : submissionResult.status === "compile_error" ? "text-yellow-400" : "text-red-400"}`}>
                              {submissionAccepted ? "Accepted" :
                               submissionResult.status === "compile_error" ? "Compilation Error" :
                        ...
                        {submissionResult.message && (
                          <p className="mt-2 text-xs text-slate-400">{submissionResult.message}</p>
                        )}
                      </div>
                    )}
                  </div>
                ...
                {outputTab === "input" && (
                  <div className="p-4">
                    ...
                    <textarea
                      ...
                      className="w-full h-28 bg-slate-800 text-slate-200 font-mono text-xs p-3 rounded-lg border border-slate-700 resize-none outline-none focus:border-blue-500 placeholder-slate-600"
                    />
                  </div>
                )}
              </div>
